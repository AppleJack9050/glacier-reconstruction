...
# monitor_gpu.sh: Monitor CPU, RAM, and GPU usage for a predefined COLMAP pipeline and record its duration
# Usage: ./monitor_gpu.sh [-i interval_in_seconds] [-o logfile] -d image_path_glob
...
# Launch the COLMAP sequence in the background
(
  cd .. || exit 1
  mkdir -p ./colmap_workdir
  colmap feature_extractor \
    --database_path ./colmap_workdir/database.db \
    --image_path "$IMAGE_GLOB"

  colmap exhaustive_matcher \
    --database_path ./colmap_workdir/database.db

  mkdir -p ./colmap_workdir/sparse
  colmap mapper \
    --database_path ./colmap_workdir/database.db \
    --image_path "$IMAGE_GLOB" \
    --output_path ./colmap_workdir/sparse
